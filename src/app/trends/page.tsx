      ...
      <ResponsiveContainer width="100%" height={140}>
        <LineChart data={dots}>
          ...
          <Line
            ...
            stroke={hex}
            strokeWidth={2}
            dot={{ fill: hex, r: 3, strokeWidth: 0 }}
            activeDot={{ r: 5 }}
            connectNulls
          />
        </LineChart>
...
export default function DashboardPage() {
  ...
  // per-dimension chart data — sorted by time for connected line
  const chartDataMap = useMemo(() => {
    const result = {} as Record<DimensionId, ChartPoint[]>;
      ...
      <div>
        ...
        {scoredSessions.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {DIMENSIONS.map((dim) => {
              const dots = chartDataMap[dim.id] ?? [];
              return (
                <DimensionChart
                  key={dim.id}
                  label={dim.label}
